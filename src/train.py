import os, time, importlib, mlflow
from sklearn.model_selection import train_test_split
from sklearn.metrics import mean_squared_error

mlflow.set_tracking_uri(os.environ["MLFLOW_TRACKING_URI"])
EXP = "taxi-duration"; NAME = "taxi-duration"
...
def main():
    ...
    with mlflow.start_run() as run:
        ...
        t=time.time(); model.fit(Xtr,ytr); ypred=model.predict(Xte); dur=time.time()-t
        rmse = mean_squared_error(yte, ypred, squared=False)
        mlflow.log_metrics({"rmse": float(rmse), "fit_seconds": float(dur)})
